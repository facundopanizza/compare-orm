...
export class CommentService {
    constructor(@Inject(COMMENT_REPOSITORY) private commentRepository: ICommentRepository) { }
    ...
    async updateComment(id: number, comment: CommentInputDto): Promise<Comment> {
        const updatedComment = await this.commentRepository.updateComment(id, comment);
        if (!updatedComment) {
            throw new Error('Comment not found');
        }
