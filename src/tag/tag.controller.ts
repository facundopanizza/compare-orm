...
export class TagController {
    ...
    @Get(':id')
    async findTagById(@Param('id', ParseIntPipe) id: number): Promise<TagOutputDto> {
        try {
    ...
    @ApiParam({ name: 'id', required: true })
    @Put(':id')
    async updateTag(@Param('id', ParseIntPipe) id: number, @Body() tag: TagInputDto): Promise<TagOutputDto> {
        try {
