...
export class PostPrismaRepository implements IPostRepository {
  ...
  async createPost(post: PostInputDto): Promise<Post> {
    const createdPost = await this.prisma.post.create({ data: post }) as unknown as Post;

    return this.toPost(createdPost);
  }

  async findPostById(id: number): Promise<Post | null> {
    const foundPost = await this.prisma.post.findUnique({ where: { id } }) as unknown as Post;
  ...
  async updatePost(id: number, post: PostInputDto): Promise<Post | null> {
    try {
      const updatedPost = await this.prisma.post.update({ where: { id }, data: post }) as unknown as Post;
