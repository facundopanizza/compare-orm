...
export class UserDrizzleRepository implements IUserRepository, OnModuleInit {
    ...
    async createUser(user: UserInputDto): Promise<User> {
        const savedUser = await this.drizzle.insert(schema.usersTable).values({
            name: user.name,
            email: user.email,
    ...
    async findUserByEmail(email: string): Promise<User | null> {
        const foundUser = await this.drizzle.select()
    ...
    async updateUser(id: number, user: UserInputDto): Promise<User | null> {
        ...
        const updatedUser = await this.drizzle.update(schema.usersTable)
            .set({
                name: user.name,
                email: user.email,
                password: user.password,
    ...
    async deleteUser(id: number): Promise<true | null> {
        ...
        await this.drizzle.delete(schema.usersTable)
            .where(eq(schema.usersTable.id, id));
